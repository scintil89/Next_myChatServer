...
namespace NLogicLib
{
	...
	ERROR_CODE PacketProcess::LOBBYWisperChat(PacketInfo packetInfo)
	{
	CHECK_START
		auto reqPkt = (NCommon::PktLOBBY_WHISPER_ChatReq*)packetInfo.pRefData;
		NCommon::PktLOBBYChatRes resPkt;
		...
		//귓속말을 받는 유저가 로비 안에 없음
		if (toUser == nullptr)
		{
			CHECK_ERROR(ERROR_CODE::LOBBY_WHISPER_CHAT_INVALID_SEND_USER);
		}
		...
		{
			CHECK_ERROR(ERROR_CODE::LOBBY_WHISPER_USER_DIFFERENT_DOMAIN);
		}
		...
		pLobby->NotifyWisperChat(pUser->GetSessioIndex(), pUser->GetID().c_str(), reqPkt->Msg, toUser->GetIndex() );
